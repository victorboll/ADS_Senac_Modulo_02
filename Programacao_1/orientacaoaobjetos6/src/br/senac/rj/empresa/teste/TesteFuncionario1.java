	...
	public static void main(String[] args) {
		...
		do {
			try {
				...
				if (gerente.equals("S")) {
					func1 = new Gerente(); // Upcasting - instancia o objeto filho em uma variável que é tipada como a mãe dele
					System.out.println("Bonificação de gerente: ");
					double bonificacao = Double.parseDouble(sc.nextLine());
					((Gerente) func1).setBonificacao(bonificacao); // Downcasting - porque na classe Funcionário não tem bonificação nesse caso, só na classe gerente, então precisa fazer o casting
				} else {

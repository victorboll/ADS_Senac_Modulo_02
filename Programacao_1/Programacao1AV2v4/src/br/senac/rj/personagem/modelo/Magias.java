	...
	public boolean atualizarMagia(int id_magia, int id_personagem, String nome_magia, String tipo_magia, int nivel_magia, String escola_magia) {
		if (!consultarMagia(id_magia, id_personagem))
			...
			try {
				...
				String sql = "UPDATE magias SET nome_magia=?, tipo_magia=?, nivel_magia=?, escola_magia=? WHERE id_magia=? AND id_personagem=?;";
				
				PreparedStatement ps = conexao.prepareStatement(sql);
				
				ps.setString(1,  nome_magia);
				ps.setString(2,  tipo_magia);
				ps.setInt(3,  nivel_magia);
				ps.setString(4, escola_magia);
				ps.setInt(5,  id_magia);

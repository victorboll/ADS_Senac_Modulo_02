...
public class JanelaConta {
	public static JFrame criarJanelaConta() {
		...
		// Define botões e a localizaçãoo deles na janela
					...
					if (titular.length() == 0) {
						...
						if (!conta.consultarConta(agencia, numero)) {
							if (!conta.cadastrarConta(agencia, numero, titular))
								JOptionPane.showMessageDialog(janelaConta, "Erro na inclus�o do titular!");
							else
								JOptionPane.showMessageDialog(janelaConta, "Inclus�o realizada!");
						} else {
							if (!conta.atualizarConta(agencia, numero, titular))
								JOptionPane.showMessageDialog(janelaConta, "Erro na atualiza��o do titular!");
							else
								JOptionPane.showMessageDialog(janelaConta, "Altera��o realizada!");
						}

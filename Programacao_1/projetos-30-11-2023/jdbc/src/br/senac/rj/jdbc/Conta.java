...
public abstract class Conta {
	...
	public boolean consultarConta(int numAgencia, int numConta) {
		...
		try {
			...
			if (!rs.isBeforeFirst()) { // Verifica se n�o est� antes do primeiro registro, se não tiver registros, ele retorna ! false = true
				System.out.println("Conta não cadastrada!");
				return false; // Conta não cadastrada
			} else {
				// Efetua a leitura do registro da tabela
				while (rs.next()) { //nem precisa desse next porque ele só pode encontrar no máximo 1
